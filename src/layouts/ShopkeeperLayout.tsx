...
export function ShopkeeperLayout() {
  ...
  const handleWebSocketEvent = useCallback((event: string, _data: any) => {
    if (event === 'new_order' || event === 'order_status_updated' || event === 'bill_generated' || event === 'payment_received') {
      fetchPendingCount();
      if (event === 'new_order' || event === 'bill_generated') playSound();
    }
  ...
  useEffect(() => {
    const handleProfileUpdate = () => {
      if (user) {
        shopService.getShopProfile().then(r => setShop(r.data)).catch(console.error);
      }
    };
  ...
  const getPageTitle = () => {
    switch (location.pathname) {
      ...
      case "/shop/menu": return "Menu Management";
      case "/shop/categories": return "Category Management";
      case "/shop/orders": return "Orders";
      case "/shop/billing": return "Billing";
      case "/shop/billing-analytics": return "Billing Analytics";
      case "/shop/qr": return "QR Code";
      case "/shop/analytics": return "Analytics";
      case "/shop/settings": return "Settings";
      case "/shop/chefs": return "Chef Management";
      case "/shop/waiters": return "Waiter Management";
      default: return "Digital Menu";
    }
  };
      ...
      <TooltipProvider delayDuration={0}>
        <div className="flex min-h-screen bg-muted/30">
          ...
          <aside className={cn(
            ...
            <div className="flex flex-col flex-1 min-h-0 bg-sidebar text-sidebar-foreground">
              {/* Logo */}
              <div className="flex h-16 items-center border-b border-sidebar-border px-6 flex-shrink-0">
                ...
                >
                  <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-sidebar-primary text-sidebar-primary-foreground font-bold overflow-hidden flex-shrink-0">
                    {shop?.logo ? <img src={shop.logo} alt={shop.name} className="w-full h-full object-cover" /> : shop?.name?.charAt(0).toUpperCase() || "D"}
                  </div>
                  {!sidebarCollapsed && (
              ...
              {/* Navigation */}
              <nav className="flex-1 min-h-0 overflow-y-auto scrollbar-sidebar space-y-1 px-3 py-4">
                {navItems.map((item) => {
                    ...
                    >
                      <div className="relative flex-shrink-0">
                        ...
                        {item.title === 'Orders' && pendingOrderCount > 0 && sidebarCollapsed && (
                          <span className="absolute -top-1.5 -right-1.5 flex h-4 min-w-[16px] items-center justify-center rounded-full bg-amber-500 px-1 text-[10px] font-bold text-white">{pendingOrderCount}</span>
                        )}
                        {item.title === 'Billing' && pendingBillCount > 0 && sidebarCollapsed && (
                          <span className="absolute -top-1.5 -right-1.5 flex h-4 min-w-[16px] items-center justify-center rounded-full bg-yellow-500 px-1 text-[10px] font-bold text-white">{pendingBillCount}</span>
                        )}
                      </div>
                      {!sidebarCollapsed && (
                        <>
                          ...
                          {item.title === 'Orders' && pendingOrderCount > 0 && (
                            <Badge className="h-5 min-w-[20px] px-1.5 text-[10px] font-bold bg-amber-500 text-white hover:bg-amber-500 rounded-full">{pendingOrderCount}</Badge>
                          )}
                          {item.title === 'Billing' && pendingBillCount > 0 && (
                            <Badge className="h-5 min-w-[20px] px-1.5 text-[10px] font-bold bg-yellow-500 text-white hover:bg-yellow-500 rounded-full">{pendingBillCount}</Badge>
                          )}
                ...
                {sidebarCollapsed ? (
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button variant="ghost" onClick={() => setSidebarCollapsed(false)} className="w-full justify-center px-2 text-sidebar-foreground hover:bg-sidebar-accent">
                  ...
                  <Button variant="ghost" onClick={() => setSidebarCollapsed(true)} className="w-full gap-3 justify-start text-sidebar-foreground hover:bg-sidebar-accent">
                    <PanelLeftClose className="h-5 w-5 flex-shrink-0" /><span>Collapse</span>
                  </Button>
                )}
                {sidebarCollapsed ? (
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button variant="ghost" onClick={handleLogout} className="w-full justify-center px-2 text-sidebar-foreground hover:bg-destructive/10 hover:text-destructive">
                  ...
                  <Button variant="ghost" onClick={handleLogout} className="w-full gap-3 justify-start text-sidebar-foreground hover:bg-destructive/10 hover:text-destructive">
              ...
              <>
                ...
                >
                  <div className="flex h-16 items-center justify-between border-b border-sidebar-border px-4 flex-shrink-0">
                    <button onClick={() => { setSidebarOpen(false); setShowShopProfile(true); }}
                    ...
                    <Button variant="ghost" size="icon-sm" onClick={() => setSidebarOpen(false)}><X className="h-5 w-5" /></Button>
                  </div>
                  <nav className="flex-1 min-h-0 overflow-y-auto scrollbar-sidebar space-y-1 px-3 py-4">
                    {navItems.map((item) => {
                      const Icon = item.icon;
                  ...
                  <div className="border-t border-sidebar-border p-3 space-y-2 flex-shrink-0">
                    ...
                    <Button variant="ghost" onClick={handleLogout} className="w-full justify-start gap-3 text-sidebar-foreground hover:bg-destructive/10 hover:text-destructive">
          ...
          <div className={cn("flex-1 transition-all duration-300", sidebarCollapsed ? "lg:pl-16" : "lg:pl-64")}>
            ...
            <header className="sticky top-0 z-30 flex h-16 items-center gap-4 border-b border-border bg-background/95 backdrop-blur-md px-4 lg:px-6">
              ...
              <div className="relative" ref={notifRef}
                onMouseEnter={() => { if (hoverTimerRef.current) clearTimeout(hoverTimerRef.current); hoverTimerRef.current = setTimeout(() => setNotifOpen(true), 200); }}
                onMouseLeave={() => { if (hoverTimerRef.current) clearTimeout(hoverTimerRef.current); hoverTimerRef.current = setTimeout(() => setNotifOpen(false), 300); }}
              >
                  ...
                  {notifOpen && (
                    <motion.div initial={{ opacity: 0, y: -8, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -8, scale: 0.95 }} transition={{ duration: 0.15 }}
                      ...
                      <div className="flex items-center justify-between px-4 py-3 border-b border-border bg-muted/30">
                        ...
                        {(pendingOrderCount + pendingBillCount) > 0 && (
                          <Badge className="h-5 px-2 text-[10px] font-bold bg-red-500 text-white hover:bg-red-500 rounded-full">{pendingOrderCount + pendingBillCount} pending</Badge>
                        )}
                      </div>
                        ...
                        {pendingOrderCount > 0 && (
                          <div>
                            <div className="flex items-center gap-2 px-4 py-2 bg-amber-500/5 border-b border-border/50">
                              ...
                              <span className="text-xs font-semibold text-amber-600 dark:text-amber-400 uppercase tracking-wider">Pending Orders</span>
                              <Badge className="ml-auto h-4 px-1.5 text-[10px] font-bold bg-amber-500 text-white hover:bg-amber-500 rounded-full">{pendingOrderCount}</Badge>
                            </div>
                            <div className="divide-y divide-border/30">
                              {recentPendingOrders.map((order) => (
                                <button key={order._id} onClick={() => { setNotifOpen(false); navigate('/shop/orders'); }}
                                  className="w-full flex items-start gap-3 px-4 py-3 hover:bg-muted/50 transition-colors text-left">
                                  <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-amber-500/10 text-amber-500 flex-shrink-0 mt-0.5"><Clock className="h-4 w-4" /></div>
                                  <div className="flex-1 min-w-0">
                                    <div className="flex items-center justify-between gap-2">
                                      <span className="text-sm font-medium truncate">{order.customerName}</span>
                                      <span className="text-xs text-muted-foreground flex-shrink-0">Table {order.tableNumber}</span>
                                    </div>
                                    <p className="text-xs text-muted-foreground mt-0.5 truncate">{order.items.map(i => `${i.quantity}x ${i.name}`).join(', ')}</p>
                                    <div className="flex items-center justify-between mt-1">
                                      <span className="text-xs font-semibold text-primary">₹{order.totalAmount.toFixed(2)}</span>
                                      <span className="text-[10px] text-muted-foreground">{new Date(order.createdAt).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}</span>
                                    </div>
                                  </div>
                                </button>
                              ))}
                            ...
                            {pendingOrderCount > 5 && (
                              <button onClick={() => { setNotifOpen(false); navigate('/shop/orders'); }}
                        ...
                        {pendingBillCount > 0 && (
                          <div>
                            <div className="flex items-center gap-2 px-4 py-2 bg-yellow-500/5 border-b border-border/50 border-t border-border/50">
                              ...
                              <span className="text-xs font-semibold text-yellow-600 dark:text-yellow-400 uppercase tracking-wider">Unpaid Bills</span>
                              <Badge className="ml-auto h-4 px-1.5 text-[10px] font-bold bg-yellow-500 text-white hover:bg-yellow-500 rounded-full">{pendingBillCount}</Badge>
                            </div>
                            <div className="divide-y divide-border/30">
                              {recentPendingBills.map((bill) => (
                                <button key={bill._id} onClick={() => { setNotifOpen(false); navigate('/shop/billing'); }}
                                  className="w-full flex items-start gap-3 px-4 py-3 hover:bg-muted/50 transition-colors text-left">
                                  <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-yellow-500/10 text-yellow-500 flex-shrink-0 mt-0.5"><Receipt className="h-4 w-4" /></div>
                                  <div className="flex-1 min-w-0">
                                    <div className="flex items-center justify-between gap-2">
                                      <span className="text-sm font-medium truncate">{bill.customerName}</span>
                                      <span className="text-xs text-muted-foreground flex-shrink-0">Table {bill.tableNumber}</span>
                                    </div>
                                    <p className="text-xs text-muted-foreground mt-0.5">Bill #{bill.billNumber}</p>
                                    <div className="flex items-center justify-between mt-1">
                                      <span className="text-xs font-semibold text-primary">₹{bill.totalAmount.toFixed(2)}</span>
                                      <span className="text-[10px] text-muted-foreground">{new Date(bill.createdAt).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}</span>
                                    </div>
                                  </div>
                                </button>
                              ))}
                            ...
                            {pendingBillCount > 5 && (
                              <button onClick={() => { setNotifOpen(false); navigate('/shop/billing'); }}
                                className="w-full text-center py-2 text-xs font-medium text-primary hover:bg-muted/50 transition-colors border-t border-border/30">
            ...
            <main className="min-h-0">
              <ShopSetupGuard>
                {getBreadcrumbs() && (
                  <div className="mb-3 px-4 sm:px-6 pt-4 sm:pt-6">{getBreadcrumbs()}</div>
                )}
        ...
        <Dialog open={showShopProfile} onOpenChange={setShowShopProfile}>
          <DialogContent className="sm:max-w-lg p-0 gap-0 overflow-hidden [&>button]:hidden">
            ...
            <div className="relative h-36 overflow-visible">
              {shop?.banner ? (
                <>
                  <img src={shop.banner} alt="Shop Banner" className="absolute inset-0 w-full h-full object-cover" />
              ...
              <div className="absolute -bottom-10 left-5 z-20">
                <div className="w-28 h-28 rounded-2xl bg-background shadow-2xl border-4 border-background overflow-hidden ring-2 ring-primary/20">
                  {shop?.logo ? <img src={shop.logo} alt={shop.name} className="w-full h-full object-cover" /> : (
            ...
            <div className="px-5 pt-12 pb-5 space-y-4">
              <div className="space-y-1">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <Badge variant="secondary" className="capitalize text-xs"><Building2 className="h-3 w-3 mr-1" />{shop?.type || "Restaurant"}</Badge>
                </div>
                {shop?.description && <p className="text-muted-foreground text-xs leading-relaxed mt-1">{shop.description}</p>}
              </div>
              <Separator />
              <div className="space-y-3">
                <h3 className="font-semibold text-xs text-muted-foreground uppercase tracking-wide">Shop Information</h3>
                <div className="grid grid-cols-2 gap-3">
                  <div className="flex items-start gap-2 p-2.5 rounded-lg bg-muted/50">
                    <div className="w-8 h-8 rounded-lg bg-primary/10 flex items-center justify-center flex-shrink-0"><Phone className="h-4 w-4 text-primary" /></div>
                    <div className="flex-1 min-w-0"><p className="text-[10px] text-muted-foreground font-medium">Phone</p><p className="text-xs font-semibold truncate">{shop?.phone || "Not set"}</p></div>
                  </div>
                  <div className="flex items-start gap-2 p-2.5 rounded-lg bg-muted/50">
                    <div className="w-8 h-8 rounded-lg bg-primary/10 flex items-center justify-center flex-shrink-0"><MapPin className="h-4 w-4 text-primary" /></div>
                    <div className="flex-1 min-w-0"><p className="text-[10px] text-muted-foreground font-medium">Address</p><p className="text-xs font-semibold truncate">{shop?.address || "Not set"}</p></div>
                  </div>
                </div>
              </div>
              <Separator />
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="flex items-start gap-2 p-2.5 rounded-lg bg-muted/50">
                    <div className="w-8 h-8 rounded-lg bg-accent/10 flex items-center justify-center flex-shrink-0"><User className="h-4 w-4 text-accent" /></div>
                    <div className="flex-1 min-w-0"><p className="text-[10px] text-muted-foreground font-medium">Owner</p><p className="text-xs font-semibold truncate">{user?.name}</p></div>
                  </div>
                  <div className="flex items-start gap-2 p-2.5 rounded-lg bg-muted/50">
                    <div className="w-8 h-8 rounded-lg bg-accent/10 flex items-center justify-center flex-shrink-0"><Mail className="h-4 w-4 text-accent" /></div>
                    <div className="flex-1 min-w-0"><p className="text-[10px] text-muted-foreground font-medium">Email</p><p className="text-xs font-semibold truncate">{user?.email}</p></div>
                  </div>
                </div>
              </div>
              <div className="pt-3">
                <Button onClick={() => { setShowShopProfile(false); navigate('/shop/settings'); }} className="w-full gap-2 h-10">
